...
const debug = require('debug')('cloudless' + ':transform');
...
function Transformer (transformerInfo) {
  console.log(JSON.stringify(transformerInfo))
  this.name = null
  this.routine = null
...
var getTransformer = function (callback) {
  if (process.env.TRANSFORM_FUNCTION) {
    ...
    try {
      var transformFunction = require(path.join(process.cwd(), process.env.TRANSFORM_FUNCTION))
      if (typeof transformFunction === 'function') {
        ...
        return callback(null, new Transformer({
          id: process.env.TRANSFORM_FUNCTION,
          routine: transformFunction
        }))
      } else {
      ...
      return callback(
      'Transformation function could not be loaded from file "' + process.env.TRANSFORM_FUNCTION + '": ' + err
      )
    }
  } else {
